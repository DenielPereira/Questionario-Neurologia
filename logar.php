...
$result = mysqli_query($con, "SELECT * FROM `usuario` WHERE `email` = '$email' AND `senha`= '$senha'");
			...
			if(empty($_POST['Email']) && empty($_POST['senha'])){
				...
				die('Como vc pretende entrar sem digitar nada?');
				
			}else if(empty($_POST['Email'])){

				die('Ops, parece que vc esqueceu de digitar seu e-mail.');

			}else if (empty($_POST['senha'])){

				die('Ops, parece que vc esqueceu de digitar a senha.');
					...
					echo "Login ou usuario invalidos";
